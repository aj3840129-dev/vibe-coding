...
async function handleApi(request, response, requestPath) {
  ...
  if (request.method === "POST" && requestPath === "/api/classroom/start") {
    ...
    store.meeting = {
      ...
      campusRoomId: payload.mode && payload.mode !== "online" ? payload.campusRoomId : null,
      title,
      topic,
      roomId: room.id,
...
workflows = require("./workflows").createWorkflows({
  database: storage, ownerId, setOwner, dataDir,
...
server.listen(port, host, () => {
  console.log(`AetherClass local URL: http://127.0.0.1:${server.address().port}`);
