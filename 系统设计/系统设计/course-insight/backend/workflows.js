...
function createWorkflows(ctx) {
  ...
  const campus = require("./campus").createCampus(ctx);
  ...
  function decorate(store, dashboard) {
    ...
    return {
      ...dashboard, ...classroomTools.decorate(store), ...campus.decorate(store), interventions, feedback: store.feedback, qaHistory: store.qaHistory,
  ...
  function exportReview(archive) {
    ...
    return [
      ...
      "## 线下教室观测", `模式：${r.meeting.mode || "online"}；教室：${r.meeting.campusRoomId || "无"}`,
      `有效观测 ${r.campus?.sampleCount || 0} 份；检测人数峰值 ${r.campus?.peakDetectedPeople ?? "未观测"}（不等于实际到课人数）`,
      ...(r.campus?.notes || []).map(n => `- ${n.message}`),
  ...
  async function prepare(req, res, route) {
    ...
    if (route === "/api/logout" && req.method === "POST") {
      ...
      if(user.role === "teacher") campus.stopOwner(user.id);
    ...
    if (await campus.handle(req,res,route)) return true;
    const store = ctx.read();
